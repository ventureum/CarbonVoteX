...
contract('Test_CarbonVote', (accounts) => {
	...
	var core, basic, namespaceBasic;
  	...
  	before(async ()=>{
		...
		namespaceBasic = await basic.getNamespace();
  		await core.setReceiver(namespaceBasic, basic.address, [wweb3.utils.sha3("register")]);
	});
	...
	describe ("testing DSAuth:", () => {
		...
		// grant sendGas permission to UNAUTHORIZED_ACCOUNT1
		it ("only ROOT_ACCOUNT can setPermissions", 
			async () =>{
				...
				await core.setPermissions (
					[wweb3.utils.sha3("register")], 
					[UNAUTHORIZED_ACCOUNT2],
					{from: UNAUTHORIZED_ACCOUNT2}
				).should.be.rejectedWith(EVMRevert);
		});
		
		// Function overloading is not supported by truffle test framework.
		// TODO: Find an alternative solution (maybe use web3 directly?)
		// Assign to vincentzhongca@gmail.com
		// it ("only ROOT_ACCOUNT can cancelPermissions",
		// 	async () => {
		// 		await core.cancelPermissions(UNAUTHORIZED_ACCOUNT1);
